...
def get_loss_function(reg_lambda=1e-2):
    """
    Returns a loss function that combines L1 loss with message-based L1 regularization.
    Args:
        reg_lambda: Weight of the L1 regularization term (default: 1e-2)
    Returns:
        A function that takes (pred, target, model, g) as arguments and returns the combined loss
    """
    def combined_loss(pred, target, model, g):
        # Base L1 loss
        base_loss = torch.sum(torch.abs(target - pred))
        
        # Message-based L1 regularization
        s1 = g.x[g.edge_index[0]]
        s2 = g.x[g.edge_index[1]]
        m12 = model.message(s1, s2)
        normalized_l1 = torch.sum(torch.abs(m12))
        
        # Normalize by batch size and number of nodes
        batch_size = g.x.size(0)
        n = g.x.size(0)
        reg_term = reg_lambda * batch_size * normalized_l1 / (n**2) * n
